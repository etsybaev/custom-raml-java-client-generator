...
public class RamlJavaClientGenerator {
    ...
    private void generate(ApiModel raml) throws JClassAlreadyExistsException, IOException {
        ...
        final JFieldVar baseUriField = containerClientClass.field(JMod.PUBLIC | JMod.STATIC, String.class, "_" + BASE_URL_FIELD_NAME, JExpr.lit(raml.getBaseUri()) );
            ...
            if (resourceParts.length > 1) {
                for (int i = 1; i < resourceParts.length; i++) {
                    ...
                    if (!resourceClasses.containsKey(resourcePath)) {
                        ...
                        JFieldVar baseUrlField = resourceClass.field(JMod.PUBLIC | JMod.STATIC, String.class,
                                    PRIVATE_FIELD_PREFIX + BASE_URL_FIELD_NAME, JExpr.lit(""));
                        ...
                        if (isURIParameter(resourceName)) {
                            ...
                            baseUrlField.init(JExpr.lit("/" + resourceName));
    ...
    public JType generatePojoFromSchema(JCodeModel codeModel, String className, String packageName, String json, SourceType sourceType) throws IOException {
        try {
            ...
            System.out.println("Generating POJO for " + packageName + className);
            File tmpFile = File.createTempFile("tmp", "json");
            FileWriter writer = new FileWriter(tmpFile);
            writer.write(json);
            writer.close();
            URL jsonSource = tmpFile.toURI().toURL();
            return schemaMapper.generate(codeModel, className, packageName, jsonSource);
        } catch (JsonParseException e) {
            System.err.println("Can not generate  " + className + " from schema since : " + e.getMessage());
